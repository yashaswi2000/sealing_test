...
static sgx_status_t SGX_CDECL sgx_ecall_seal_sample(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_ecall_seal_sample_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_ecall_seal_sample_t* ms = SGX_CAST(ms_ecall_seal_sample_t*, pms);
	...
	ms->ms_retval = ecall_seal_sample(_in_value);
	if (_in_value) {
		if (memcpy_s(_tmp_value, _len_value, _in_value, _len_value)) {
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}
	}
...
static sgx_status_t SGX_CDECL sgx_ecall_example(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_ecall_example_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_ecall_example_t* ms = SGX_CAST(ms_ecall_example_t*, pms);
	sgx_status_t status = SGX_SUCCESS;
	...
	ecall_example((const char*)_in_str);
...
SGX_EXTERNC const struct {
	...
	4,
	{
		{(void*)(uintptr_t)sgx_ecall_seal_sample, 0, 0},
		{(void*)(uintptr_t)sgx_ecall_example, 0, 0},
...
sgx_status_t SGX_CDECL ocall_seal_sample(const char* str)
{
	sgx_status_t status = SGX_SUCCESS;
	size_t _len_str = str ? strlen(str) + 1 : 0;

	ms_ocall_seal_sample_t* ms = NULL;
	size_t ocalloc_size = sizeof(ms_ocall_seal_sample_t);
	void *__tmp = NULL;
	...
	ms = (ms_ocall_seal_sample_t*)__tmp;
	__tmp = (void *)((size_t)__tmp + sizeof(ms_ocall_seal_sample_t));
	ocalloc_size -= sizeof(ms_ocall_seal_sample_t);
...
sgx_status_t SGX_CDECL ocall_sample(const uint8_t* A)
{
	sgx_status_t status = SGX_SUCCESS;
	size_t _len_A = 32 * sizeof(uint8_t);

	ms_ocall_sample_t* ms = NULL;
	size_t ocalloc_size = sizeof(ms_ocall_sample_t);
	void *__tmp = NULL;
	...
	ms = (ms_ocall_sample_t*)__tmp;
	__tmp = (void *)((size_t)__tmp + sizeof(ms_ocall_sample_t));
	ocalloc_size -= sizeof(ms_ocall_sample_t);

	if (A != NULL) {
		ms->ms_A = (const uint8_t*)__tmp;
